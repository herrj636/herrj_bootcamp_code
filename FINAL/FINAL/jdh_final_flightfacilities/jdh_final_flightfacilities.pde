...
int txt = 100;
PFont computer;
String band = "FLIGHT FACILITIES"; //INSERT BAND NAME
String place = "WEBSTER HALL - NEW YORK, NY"; //INSERT GIG LOCATION
String date = "OCTOBER 11, 2016"; //INSERT GIG DATE
...
void setup() {
  ...
  textAlign(CENTER);
  player = minim.loadFile("CraveYou.mp3", 1024);
  player.play();
}
...
void draw() {
  ...
  for (int b = 180; b > 0; b--) {
    ...
    if (player.mix.get(b)*200 > 25) {
      stroke(color3, 100, 100,50);
    } else {
      stroke(color4, 100, 100,50);
    }
  ...
  for(int c = 720; c > 0; c--) {
    ...
    if(player.mix.get(c)*500 > 25) {
      stroke(color5,100,100,50);
    }
    else {
      stroke(color6,100,100,50);
    }
