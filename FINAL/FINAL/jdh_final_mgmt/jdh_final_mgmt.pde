...
int txt = 100;
PFont computer;
String band = "MGMT"; //INSERT BAND NAME
String place = "GLASTONBURY - PILTON, SOMERSET"; //INSERT GIG LOCATION
String date = "JUNE 28, 2014"; //INSERT GIG DATE
...
void setup() {
  ...
  textAlign(CENTER);
  player = minim.loadFile("mgmt.mp3", 1024);
  player.play();
}
...
void draw() {
  ...
  for (int a = 0; a < 60; a++) {
    ...
    if (player.mix.get(a)*200 > 80) {
      stroke(color1, 100, 100,5);
    } else {
      stroke(color2, 100, 100,5);
    }
  ...
  for (int b = 180; b > 0; b--) {
    ...
    if (player.mix.get(b)*200 > 25) {
      stroke(color3, 100, 100,3);
    } else {
      stroke(color4, 100, 100,3);
    }
  ...
  for(int c = 720; c > 0; c--) {
    ...
    if(player.mix.get(c)*500 > 25) {
      stroke(color5,100,100,2);
    }
    else {
      stroke(color6,100,100,2);
    }
